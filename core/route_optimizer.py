...
from .geo_graph import road_graph_for_city, shortest_distance_km, haversine_dist
...
def pairwise_distance_matrix(city: str, pois: List[Dict]) -> Tuple[list, list, list]:
    ...
    Falls back to haversine distance when routing fails.
    """
    ...
    for i in range(n):
        for j in range(i+1, n):
            ...
            # If routing fails (infinity), use straight-line distance as fallback
            if math.isinf(d) or d > 100:  # Cap at 100km for reasonable city travel
                lat1, lon1 = coords[i]
                lat2, lon2 = coords[j]
                d = haversine_dist(lat1, lon1, lat2, lon2) / 1000.0  # Convert to km
                # Add 30% penalty for straight-line (actual roads are longer)
                d *= 1.3
